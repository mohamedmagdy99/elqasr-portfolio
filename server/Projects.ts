...
export const getAllProjects = async ({
    ...
    const res = await fetch(`http://192.168.1.16:3000/api/projects?${params.toString()}`);
...
export const getSingleProject = async (id: string) => {
    try {
        const res = await fetch(`http://192.168.1.16:3000/api/projects/${id}`, {
            cache: "no-store",
        });
...
export const CreateProject = async (formData: FormData) => {
    ...
    const res = await fetch("http://192.168.1.16:3000/api/projects", {
        method: "POST",
        body: formData,
        headers: {
...
export const UpdateProject = async (id: string, formData: FormData) => {
    ...
    const res = await fetch(`http://192.168.1.16:3000/api/projects/${id}`, { // ✅ include ID in URL
        method: "PUT",
        body: formData,
        headers: {
